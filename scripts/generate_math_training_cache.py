...
from eapae_agent_sys.data_processing.math_loader import load_math_dataset, prepare_math_for_training
...
def worker(work_queue, result_queue, heartbeat_dict, incorrect_queue):
    ...
    config_loader = ConfigLoader(agent_library_file="0_agent_library_math.yml", training_params_file="3_training_params_math.yml")
    llm_api = LLM_API()
        ...
        if not candidate_agents:
            ...
            if is_feasible:
                final_context, actual_cost = execution_engine.execute_hybrid(
                    agent_pool, budget, task['question'], collaboration_pattern=pattern, dataset_type="math"
                )
                success_code = evaluate_math_success(final_context, task['answer'])
...
def generate_cache_single_pattern_budget(args, pattern_idx: int, budget_idx: int):
    ...
    try:
        train_dataset = load_math_dataset('data/math', split='sampled_train')
        if train_dataset:
            train_dataset = [prepare_math_for_training(sample) for sample in train_dataset]
        else:
            raise ValueError("Sampled data is empty")
    except Exception as e:
        print(f"Failed to load sampled data: {e}")
        train_dataset = load_math_dataset('data/math', split='train')
        if train_dataset:
            train_dataset = [prepare_math_for_training(sample) for sample in train_dataset]
        else:
            raise ValueError("Unable to loadMATHdataset")
    ...
    return output_path
def load_failed_tasks_from_file(file_path: str):
    """
    Load all failed tasks from existing offline dataset file
    Args:
        file_path: containing failed tasksjsonlfile path
    Returns:
        list: list containing failed task information
    """
    failed_tasks = []
    ...
    total_lines = 0
    timeout_failures = 0
    print(f"Loading failed tasks from: {file_path}")
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            total_lines += 1
            try:
                entry = json.loads(line.strip())
                if (entry.get('reason') == 'worker_timeout_or_unretrieved_result' or 
                    (not entry.get('is_correct', True) and entry.get('planning_feasible', True) == False)):
                    failed_tasks.append(entry)
                    timeout_failures += 1
            except json.JSONDecodeError:
                print(f"Warning: Skip invalidJSONline: {line[:100]}...")
                continue
    print(f"  Total entries: {total_lines}")
    print(f"  Timeout failed tasks: {timeout_failures}")
    print(f"  Ready to re-run: {len(failed_tasks)}")
    return failed_tasks
def reconstruct_work_items_from_failed_tasks(failed_tasks, config_loader):
    """
    Reconstruct from failed task listwork_items
    Args:
        failed_tasks: Failed task list
        config_loader: Configuration loader
    Returns:
        tuple: (work_items, work_items_map, datasets_needed)
    """
    work_items = []
    work_items_map = {}
    datasets_needed = set()
    ...
    for failed_task in failed_tasks:
        ...
        budget = failed_task['budget']
        action = failed_task['action']
        try:
            parts = work_id.split('_')
            if len(parts) >= 6:
                task_idx = int(parts[1])
                budget_idx = int(parts[3])
                pattern_idx = int(parts[5])
            else:
                print(f"Warning: Unable to parsework_idformat: {work_id}")
                continue
        except (ValueError, IndexError):
            print(f"Warning: Unable to parsework_id: {work_id}")
            continue
        datasets_needed.add(task_idx)
        task_data = {
            'question': task_description,
            'answer': 'unknown'
        }
        if action < len(all_patterns):
            pattern = all_patterns[action]
        else:
            print(f"Warning: Invalidpatternindex {action}, Use defaultpattern")
            pattern = all_patterns[0]
        work_item = (work_id, task_data, budget, pattern, action, behavior_log_prob)
        work_items.append(work_item)
        work_items_map[work_id] = work_item
    print(f"Task index range to load from dataset: {min(datasets_needed) if datasets_needed else 'N/A'} - {max(datasets_needed) if datasets_needed else 'N/A'}")
    return work_items, work_items_map, datasets_needed
def rerun_failed_tasks(args):
    """
    Re-run failed tasks
    Args:
        args: Command line arguments，containingrerun_file_path
    """
    print("=== Re-run failed tasks mode ===")
    failed_tasks = load_failed_tasks_from_file(args.rerun_file_path)
    if not failed_tasks:
        print("No failed tasks found that need re-running")
        return
    mp.set_start_method("spawn", force=True)
    config_loader = ConfigLoader(agent_library_file="0_agent_library_math.yml", training_params_file="3_training_params_math.yml")
    params = config_loader.params
    work_items, work_items_map, datasets_needed = reconstruct_work_items_from_failed_tasks(failed_tasks, config_loader)
    if not work_items:
        return
    try:
        train_dataset = load_math_dataset('data/math', split='sampled_train')
        if not train_dataset:
            train_dataset = load_math_dataset('data/math', split='train')
        if not train_dataset:
            raise ValueError("Unable to load anyMATHdataset")
        train_dataset = [prepare_math_for_training(sample) for sample in train_dataset]
        updated_count = 0
        for work_id, (_, task_data, budget, pattern, action, behavior_log_prob) in work_items_map.items():
            try:
                parts = work_id.split('_')
                task_idx = int(parts[1])
                if task_idx < len(train_dataset):
                    task_data['answer'] = train_dataset[task_idx]['answer']
                    updated_count += 1
                else:
                    print(f"Warning: Task index {task_idx} exceeds dataset range ({len(train_dataset)})")
            except (ValueError, IndexError):
                print(f"Warning: Unable to parsework_id {work_id}task index in")
        print(f"Successfully updated {updated_count} correct answers for tasks")
    except Exception as e:
    ...
    monitor_thread.start()
    rerun_dataset = []
    incorrect_rerun_data = []
    total_work_items = len(work_items)
    try:
        with tqdm(total=total_work_items, desc="Re-run failed tasks") as pbar:
            while pbar.n < total_work_items:
                try:
                    experience = result_queue.get(timeout=5)
                    rerun_dataset.append(experience)
                    pbar.update(1)
                    try:
                        while True:
                            incorrect_exp = incorrect_queue.get_nowait()
                            incorrect_rerun_data.append(incorrect_exp)
                    except queue.Empty:
                        pass
                except queue.Empty:
                    if not any(p.is_alive() for p in workers):
        ...
        try:
            ...
            pass
        successful_ids = {exp.get('work_id') for exp in rerun_dataset if exp.get('work_id')}
        all_ids = set(work_items_map.keys())
        still_failed_ids = all_ids - successful_ids
        if still_failed_ids:
            print(f"[Rerun] Still have {len(still_failed_ids)} tasks failedmarked as negative samples")
            for failed_id in sorted(list(still_failed_ids)):
                ...
                failure_experience = {
                    ...
                    "planning_feasible": False,
                    "behavior_log_prob": behavior_log_prob,
                    "reason": "worker_timeout_or_unretrieved_result_rerun"
                }
                rerun_dataset.append(failure_experience)
        stop_event.set()
        monitor_thread.join(timeout=5)
        for p in workers:
            if p.is_alive():
        ...
        for p in workers:
            p.join()
    base_path, ext = os.path.splitext(args.rerun_file_path)
    output_path = f"{base_path}_rerun{ext}"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, 'w', encoding='utf-8') as f:
        for entry in rerun_dataset:
            f.write(json.dumps(entry) + "\n")
    success_count = sum(1 for exp in rerun_dataset if exp.get('is_correct', False))
    still_timeout_count = sum(1 for exp in rerun_dataset if exp.get('reason', '').endswith('_rerun'))
    print(f"\n=== Re-run result statistics ===")
    print(f"Original failed tasks: {len(failed_tasks)}")
    print(f"Re-run tasks: {len(work_items)}")
    print(f"Successful tasks this time: {success_count}")
    print(f"Still timeout tasks: {still_timeout_count}")
    print(f"Success rate: {success_count/len(work_items)*100:.1f}%")
    print(f"Results saved to: {output_path}")
    if incorrect_rerun_data:
        incorrect_output_path = f"{base_path}_rerun_incorrect{ext}"
        with open(incorrect_output_path, 'w', encoding='utf-8') as f:
            for entry in incorrect_rerun_data:
                f.write(json.dumps(entry) + "\n")
        print(f"Error experience data ({len(incorrect_rerun_data)} entries) saved to: {incorrect_output_path}")
def main(args):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_samples", type=int, default=1000, help="Number of tasks. -1 for all.")
    parser.add_argument("--begin_with", type=int, default=0, help="Starting index for batch processing (0-based).")
    parser.add_argument("--num_budget_steps", type=int, default=3, help="Number of budget levels per task.")
    ...
    parser.add_argument("--rerun_file_path", type=str, default=None, help="Path to a JSONL file containing failed tasks to rerun.")
    ...
    elif args.rerun_file_path:
        rerun_failed_tasks(args)
    else:
        main(args)