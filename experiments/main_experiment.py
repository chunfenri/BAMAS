...
Evaluates the trained system on GSM8K test dataset across different budgets using multiprocessing.
"""
import torch
import random
import os
            ...
            if not candidate_agents:
                ...
                if is_feasible:
                    final_context, actual_cost = execution_engine.execute_hybrid(
                        agent_pool, budget, task['question'], collaboration_pattern=selected_pattern
                    )
...
def main():
    parser = argparse.ArgumentParser(description="Main experiment: Parallel full pipeline inference on GSM8K test set")
    ...
    parser.add_argument("--num_budget_steps", type=int, default=5, help="Number of budget levels")
    ...
    print("="*80)
    print("MAIN EXPERIMENT: PARALLEL FULL PIPELINE INFERENCE")
    print("="*80)
    ...
    np.random.seed(seed)
    test_dataset_path = params['data']['gsm8k_test_path']
    test_dataset = load_gsm8k_dataset(path=test_dataset_path)
    if args.beginwith > 0:
    ...
    if args.model_path:
        ...
        model_path = os.path.join(params['outputs']['checkpoints_dir'], "high_level_policy_offline_best.pth")
